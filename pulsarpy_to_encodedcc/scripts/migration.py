...
def download_fastq_file(accession, library, read_num):
    '''
    ...
    elif os.path.isdir(path_to_sreq) and len(os.listdir(path_to_sreq)) == 0:
        print ("Clean up for a failed download")
...
def download_and_compute_md5sum_worker(accession, library):
    read1_file = download_fastq_file(accession, library, '1')
    read1_file_md5sum = fastq_md5sum(read1_file)

    read2_file = download_fastq_file(accession, library, '2')
    read2_file_md5sum = fastq_md5sum(read2_file)

    return read1_file, read1_file_md5sum, read2_file, read2_file_md5sum

def download_and_compute_md5sum(accession, library):
    return "read1_file", "read1_file_md5sum", "read2_file", "read2_file_md5sum"
...
def extract_chip_meta(accession, fout):
    ...
    if count > 1:
        ...
        library = extract_library_meta(library_id, 'wild')
        f1, md5sum1, f2, md5sum2 = download_and_compute_md5sum(accession, library)
        library['read1_file'] = f1
        library['read1_file_md5sum'] = md5sum1
        library['read2_file'] = f2
        library['read2_file_md5sum'] = md5sum2
        
        write_out(accession, fout, library)
    else:
    ...
    if count >= 1:
        for lib in chip['replicate_ids']:
            library = extract_library_meta(lib, 'biological')
            f1, md5sum1, f2, md5sum2 = download_and_compute_md5sum(accession, library)
            library['read1_file'] = f1
            library['read1_file_md5sum'] = md5sum1
            library['read2_file'] = f2
    ...
    if count > 1:
        ...
        library = extract_library_meta(library_id, 'control')
        f1, md5sum1, f2, md5sum2 = download_and_compute_md5sum(accession, library)
        library['read1_file'] = f1
        library['read1_file_md5sum'] = md5sum1
        library['read2_file'] = f2
...
def extract_library_meta(library_id, replicate_type):
    '''
    ...
    # sequencing request
    if len(library['sequencing_request_ids']) != 1:
        raise Exception('Library {} has zero or more than one sequencing requests.'.format(library_id))
    
    sreq = library['sequencing_request_ids'][0]
    sreq = models.SequencingRequest.find_by({'id': sreq})
    ...
    # sequencing result
    if len(library['sequencing_result_ids']) != 1:
        raise Exception('Library {} has zero or more than one sequencing results.'.format(library_id))
    
    sres = library['sequencing_result_ids'][0]
    sres = models.SequencingResult.find_by({'id': sres})
